...
from config import get_fundamentals_paths, ensure_fundamentals_directories
...
parser.add_argument('--api-key', default='NI9UUKNAPMG66IY5', help='Alpha Vantage API key')
args = parser.parse_args()

# Configuration
api_key = args.api_key
symbol = args.symbol.upper()
paths = ensure_fundamentals_directories(symbol)

# Fetch ACTUAL EARNINGS data (NO ESTIMATES - fundamentals only!)
print(f"📊 Fundamentals-only veri çekiliyor: {symbol}...")
print("   1/4 Gerçekleşen veriler (NO estimates)...")
...
# Build estimates list from EARNINGS data (not from ESTIMATES API!)
estimates = []
for quarterly in data_earnings.get('quarterlyEarnings', []):
    estimates.append({
        'date': quarterly['fiscalDateEnding'],
        'horizon': 'historical fiscal quarter',
        'eps_estimate_average': '',  # No estimates!
        'eps_estimate_high': '',
        'eps_estimate_low': '',
...
print(f"   ✓ {len(estimates)} row oluşturuldu (quarterly + annual, NO estimates)")

# Fetch INCOME STATEMENT data (for margins, revenue, EBITDA)
print("   2/4 Gelir tablosu verileri (Income Statement)...")
...
print("   3/4 Nakit akışı verileri (Cash Flow)...")
...
print("   4/4 Fiyat verileri (Price History)...")
...
for year in sorted(years_data.keys(), reverse=True):
    ...
    # SKIP Q4 calculation for fundamentals-only (no estimates!)
    if year_info['fiscal_year'] and len(year_info['quarters']) >= 2:
        # Skip if no estimates
        if not year_info['fiscal_year'].get('eps_estimate_average'):
            continue
...
def apply_elo_ratings(rows):
    ...
    for i, row in enumerate(rows_sorted):
        ...
        n_analysts_raw = row.get('eps_estimate_analyst_count', 10)
        n_analysts = float(n_analysts_raw) if n_analysts_raw else 10
        horizon = row.get('horizon', '')
        ...
        if actual_eps and estimate_eps and is_quarterly:
            ...
            if 'fiscal year' in horizon.lower() and actual_eps:
                ...
                n_analysts_for_k = n_analysts_raw if n_analysts_raw else 10
                row['K_adaptive'] = round(calculate_adaptive_K(float(n_analysts_for_k)), 2)
    ...
    for i, row in enumerate(quarterly_rows):
        ...
        if i >= 7:
            ...
            for metric in growth_metrics:
                # Sum last 4 quarters
                current_sum = sum(
                    float(quarterly_rows[j].get(metric, 0) or 0)
                    for j in range(i - 3, i + 1)
                    if quarterly_rows[j].get(metric)
                )
